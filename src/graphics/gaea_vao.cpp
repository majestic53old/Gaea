...
namespace gaea {
	...
	namespace graphics {
		...
		namespace vao {
			...
			_base::_base(void) :
				gaea::graphics::base(GL_OBJECT_VAO)
			{
				return;
			}
				...
				)
			{
				...
				std::vector<gaea::graphics::vbo::base>::const_iterator vbo_iter;
				...
				if(!object.m_vbo.empty()) {
					...
					for(vbo_iter = object.m_vbo.begin(); vbo_iter != object.m_vbo.end();
						...
						result << gaea::graphics::vbo::base::as_string(*vbo_iter, verbose);
					}
				...
				)
			{
				...
				m_vbo.push_back(gaea::graphics::vbo::base(data, size, target, usage));
			...
			{
				return gaea::graphics::vao::base::as_string(*this, verbose);
			}
		}
	}
}
